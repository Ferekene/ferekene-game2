<script lang="ts">
	...
	import { requestAuthenticate, API_AMOUNT_MULTIPLIER } from '../lib/rgsRequests';
	import { stateUrlDerived, isValidSession } from '../lib/stateUrl';
	...
	const authenticate = async () => {
		try {
			...
			if (authenticateData?.balance) {
				...
				stateRGS.setBalance(
					authenticateData.balance.amount / API_AMOUNT_MULTIPLIER,
					authenticateData.balance.currency
				);

				console.log('[Auth] Balance set:', authenticateData.balance.amount / API_AMOUNT_MULTIPLIER);
			}
			...
			if (authenticateData?.config) {
				stateConfig.update((state) => ({
					...
					betAmountOptions: (authenticateData.config.betLevels || []).map(
				...
				if (config.betAmountOptions.length > 0) {
					...
					console.log('[Auth] Config loaded:', {
						betLevels: config.betAmountOptions,
